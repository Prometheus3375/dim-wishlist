from wishlist import *


class ArcLogic(RollDefinition):
    """
    Arc Auto Rifle, Adaptive Frame, Anti-Barrier
    Source: The Moon
    https://www.light.gg/db/items/38064944
    https://destiny.report/w/38064944
    """
    item = Item('Arc Logic', hash=38064944)


class ChrysuraMelo(RollDefinition):
    """
    Arc Auto Rifle, Support Frame, Anti-Overload
    Source: The Shattered Throne
    https://www.light.gg/db/items/1750388538
    https://destiny.report/w/1750388538
    """
    item = Item('Chrysura Melo', hash=1750388538)


class Prosecutor(RollDefinition):
    """
    Arc Auto Rifle, Precision Frame, Anti-Barrier
    Source: "Prophecy" Dungeon
    https://www.light.gg/db/items/1091550016
    https://destiny.report/w/1091550016
    """
    item = Item('Prosecutor', hash=1091550016)


class ShadowPrice(RollDefinition):
    """
    Arc Auto Rifle, Precision Frame, Anti-Barrier
    Source: Commander Zavala
    https://www.light.gg/db/items/3142989221
    https://destiny.report/w/3142989221
    """
    item = Item('Shadow Price', hash=3142989221)


class SnipehuntMk47(RollDefinition):
    """
    Arc Auto Rifle, Lightweight Frame, Anti-Overload
    Source: Sparrow Racing League
    https://www.light.gg/db/items/193009988
    https://destiny.report/w/193009988
    """
    items = [
        Item('Snipehunt Mk. 47', hash=193009988),
        Item('Snipehunt Mk. 47', hash=2587567543),
        ]


class SorrowsVerse(RollDefinition):
    """
    Arc Auto Rifle, Adaptive Frame, Anti-Barrier
    Source: Crucible
    https://www.light.gg/db/items/775830132
    https://destiny.report/w/775830132
    """
    items = [
        Item("Sorrow's Verse", hash=775830132),
        Item("Sorrow's Verse", hash=2557966895),
        ]


class Trendsetter(RollDefinition):
    """
    Arc Auto Rifle, Adaptive Frame, Anti-Barrier
    Source: World
    https://www.light.gg/db/items/2863808104
    https://destiny.report/w/2863808104
    """
    item = Item('Trendsetter', hash=2863808104)


class VSPyroelectricPropellant(RollDefinition):
    """
    Arc Auto Rifle, Adaptive Frame, Anti-Barrier
    Source: Vesper's Host
    https://www.light.gg/db/items/2460368549
    https://destiny.report/w/2460368549
    """
    item = Item('VS Pyroelectric Propellant', hash=2460368549)
